...
class CpmPointMutationList(BaseSpider):
    """
    CPM - 报价单 - 点突变 - 列表数据
    """
    name = 'cpm_point_mutation_list'

    table_comment = 'CPM - 报价单点突变列表数据'
    ...
    item_map = {
        ...
        'mutation_sequence_type': 'mutation_sequence_type',
        'template_name': 'template_name',
        'template_sequence_type': 'template_sequence_type',
    ...
    url_pre = 'https://cpm.wuxibiologics.com/portaldatacollectionapi/GeneSynthesis/GetPointMutationList?reportId={}&version={}'
    headers = {
    ...
    def parse(self, response, **kwargs):
        ...
        for item in data_list:
            data_id = item.get('id')
            mutation_name = item.get('MutationName')
            mutation_sequence_type = item.get('MutationSequenceType')
            template_name = item.get('TemplateName')
            template_sequence_type = item.get('TemplateSequenceType')
...
if __name__ == '__main__':
    ...
    run_spider('cpm_point_mutation_list', proxy_type='', settings=custom_settings)
